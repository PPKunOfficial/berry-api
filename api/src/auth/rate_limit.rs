...
impl RateLimitState {
    ...
    fn cleanup(&mut self) {
        ...
        // 清理超过1分钟的记录
        self.minute_requests.retain(|&time| now.duration_since(time) < Duration::from_secs(60));
        
        // 清理超过1小时的记录
        self.hour_requests.retain(|&time| now.duration_since(time) < Duration::from_secs(3600));
        
        // 清理超过1天的记录
        self.day_requests.retain(|&time| now.duration_since(time) < Duration::from_secs(86400));
    ...
    fn check_rate_limit(&mut self, limit: &RateLimit) -> Result<()> {
        ...
        if self.minute_requests.len() >= limit.requests_per_minute as usize {
            anyhow::bail!("Rate limit exceeded: {} requests per minute", limit.requests_per_minute);
        }

        // 检查每小时限制
        if self.hour_requests.len() >= limit.requests_per_hour as usize {
            anyhow::bail!("Rate limit exceeded: {} requests per hour", limit.requests_per_hour);
        }

        // 检查每天限制
        if self.day_requests.len() >= limit.requests_per_day as usize {
            anyhow::bail!("Rate limit exceeded: {} requests per day", limit.requests_per_day);
        }
...
impl RateLimitService {
    ...
    pub async fn check_rate_limit(&self, user_id: &str, limit: &RateLimit) -> Result<()> {
        ...
        let state = states.entry(user_id.to_string()).or_insert_with(RateLimitState::new);
    ...
    pub async fn cleanup_expired_states(&self) {
        ...
        states.retain(|_, state| {
            now.duration_since(state.last_cleanup) < Duration::from_secs(86400)
        });
