...
pub async fn metrics_middleware(
    ...
    if let Some(state) = state {
        // 使用批量指标收集器记录HTTP请求
        state.batch_metrics.record_http_request(
            &method,
            &path,
            status_code,
            duration,
        ...
        if let Some(ref prometheus_metrics) = state.prometheus_metrics {
            ...
            prometheus_metrics.http_request_duration_seconds
...
pub fn record_backend_request_metrics(
    ...
    // 使用批量指标收集器记录后端请求
    state.batch_metrics.record_backend_request(
        provider,
        model,
        success,
        latency,
...
pub fn record_health_check_metrics(
    ...
    // 使用批量指标收集器记录健康检查
    state.batch_metrics.record_health_check(
        backend_key,
        healthy,
        check_duration,
    );
...
pub fn record_cache_metrics(
    ...
    // 使用批量指标收集器记录缓存指标
    state.batch_metrics.record_cache_metric(cache_type, operation);
}
