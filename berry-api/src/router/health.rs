...
pub async fn simple_health_check(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let enabled_providers = config.providers.values().filter(|p| p.enabled).count();
    let total_models = config.models.len();
    let enabled_models = config.models.values().filter(|m| m.enabled).count();
    ...
    for model_mapping in config.models.values() {
        if model_mapping.enabled {
            for backend in &model_mapping.backends {
                if backend.enabled {
                    total_backends += 1;
                    if metrics.is_healthy(&backend.provider, &backend.model) {
                        healthy_backends += 1;
                    }
                }
            }
